...
## 効果音と振動
音声ファイルは使わず Web Audio API で合成しています（`static/app.js` の `sfx`）。ゲーム進行（カード選択・決定・新しいお題・公開・勝ち負け・残り5秒・優勝）に加え、画面操作（ボタン全般、部屋の入退室、誰かの入退室、ゲーム開始、チャット送受信、エラー、小窓の開閉、設定の切り替え）にも音が付きます。候補の試聴は `static/sfx_preview.html`。
...
- 国旗一覧: 検索（ひらがな・カタカナ・英語・首都）、地域絞り込み、並び替え、「覚えた」チェック（端末に保存）
