...
2. 名前を入れて「部屋を作る」→ 部屋の名前（既定はホストの名前）を友だちに伝える。友だちはトップの「参加する」に部屋の名前を入れるだけ。招待リンクでも参加できる（内部では4文字のコードで部屋を識別）
3. ホストが部屋の名前と公開/非公開を設定（ラウンド数7・手札8枚・全カテゴリは固定。変更はサーバー側の DEFAULT_SETTINGS で可能）
4. 各ラウンド全員が同時に1枚選ぶ → 全員出そろったら一斉に公開 → お題に合った順に点（出した人がN人なら1位N点・2位N-1点…最下位1点。同じ値は同じ順位・同じ点。データのない国は1点）→ 8秒後に自動で次のラウンドへ。回答は1問30秒で、時間切れは手札からランダムに出ます
...
| | 国旗王（原作） | GeoKing |
|---|---|---|
| お題 | 面積・人口・GDP・正式名称・南北・軍事費など | 原作系＋気候・宗教・社会の4カテゴリ31種 |
| カード | 197カ国の物理カード | 同じ197カ国（国連193＋バチカン・コソボ・クック諸島・ニウエ） |
| 判定 | 裏面を見て手動で比較 | 自動判定・順位表示・全データ閲覧 |
| 人数 | 2〜8人 | 2〜8人（ボット可）＋オンライン再接続 |

## ひとりで国旗クイズ（`static/quiz.html`）
- サーバー不要（国データは `/api/meta`）。10問・4択。国旗モード（国名→国旗）と国名モード（国旗→国名）
- まちがい選択肢は同じ地域の国から優先して出す。結果画面にまちがえた国を並べ、タップで国データ（記録の保存はしない）
- 入口はトップの「ひとりで国旗クイズ」と図鑑のタブ横

## 言語（日本語・英語）
- 文言は `static/i18n.js` の辞書（`I18N.ja` / `I18N.en`）にまとめています。HTML は `data-i18n` 属性、JS は `t('key')` で参照
- 言語はヘッダーの「EN／日本語」ボタンで切り替え（保存）。初回はブラウザの言語で判定
- お題・カテゴリ・指標名の英語は `prompts.py` 末尾の `PROMPT_EN` / `CATEGORY_EN` / `FIELD_EN`。国名は `name_en` / `name_official_en`
- サーバーのエラー・通知は `code`（例: `room_not_found`）を付けて送り、クライアントが言語に合わせて表示
...
- ランキング: 31のお題ごとに全197か国を順位付きで一覧。昇順／降順の反転、地域絞り込み（順位は全体での順位を表示）
